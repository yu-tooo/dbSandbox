...
- それぞれのファイルがどのテーブル・どの学習用途に使用するものなのかは、このREADMEで記録しておく
...
| テーブル | DDL（`initdb.d/`） | データ（`data/`） | 用途 |
|---|---|---|---|
| address | `01_address.sql` | `address.csv` | SQL実践入門 41ページ付近。実行計画やSQLコマンドの挙動確認用 |
| address2 | `02_address2.sql` | `address2.csv` | address との UNION / INTERSECT などを試す用 |

> DDLを `initdb.d/` に追加した場合は、上の表の該当列にファイル名を追記してください。

## データファイルの補足

### address.csv
1, 2行目は直接 INSERT したため、このファイルは3行目以降のデータです。
